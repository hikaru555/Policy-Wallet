  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 flex flex-col min-h-[400px]">
           <div className="flex items-center justify-between mb-10 border-b border-slate-50 pb-6">
             <div>
                ...
                <p className="text-sm text-slate-400 font-medium mt-1">{t.trustedContactsDesc}</p>
             </div>
             <button 
           ...
           {profile.isPublicProfile ? (
             <div className="space-y-8 animate-in zoom-in-95 duration-300">
               <div className="p-6 bg-slate-50 rounded-[1.75rem] border border-slate-200 break-all relative group">
                 <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2.5 px-1">{t.portfolioAccessUrl}</p>
               ...
               <div className="flex flex-col sm:flex-row gap-4">
                 <button onClick={copyToClipboard} className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-indigo-100 transition-all active:scale-95 flex items-center justify-center gap-3"><span className="text-lg">📋</span> {t.copyLink}</button>
                 <button className="flex-1 py-4 bg-slate-900 text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3"><span className="text-lg">📱</span> {t.qrCode}</button>
               </div>
               {showToast && <div className="text-center animate-in fade-in slide-in-from-bottom-2 duration-300"><p className="text-[11px] font-black text-emerald-600 uppercase tracking-[0.2em] bg-emerald-50 inline-block px-4 py-1.5 rounded-full border border-emerald-100">{t.linkCopied}</p></div>}
             </div>
           ) : (
        ...
        <div className={`bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 relative ${!isPro ? 'overflow-hidden' : ''} min-h-[400px]`}>
           {!isPro && (
             <div className="absolute inset-0 z-20 bg-white/60 backdrop-blur-[3px] flex items-center justify-center p-10 text-center">
                <div className="max-w-xs animate-in slide-in-from-bottom-4">
                  ...
                  <button className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black tracking-widest uppercase text-xs shadow-xl shadow-indigo-100 active:scale-95 transition-all">{t.upgradeNow}</button>
                </div>
             </div>
           )}
           ...
           <form onSubmit={addContact} className="flex gap-3 mb-10">
             <input type="email" placeholder="contact@email.com" className="flex-1 p-4 bg-slate-50 border border-slate-200 rounded-2xl text-[15px] focus:ring-2 focus:ring-indigo-500 outline-none font-medium" value={newContact} onChange={(e) => setNewContact(e.target.value)} />
           ...
           <div className="space-y-4">
             {profile.sharedWithEmails?.length ? profile.sharedWithEmails.map(email => (
               <div key={email} className="flex items-center justify-between p-4 bg-slate-50 hover:bg-white rounded-[1.5rem] border border-slate-100 hover:shadow-md transition-all group">
                 <div className="flex items-center gap-4"><div className="w-11 h-11 bg-white rounded-xl flex items-center justify-center text-xl shadow-sm border border-slate-100 transition-transform group-hover:scale-110">👤</div><span className="text-[15px] font-bold text-slate-700">{email}</span></div>
                 <button onClick={() => removeContact(email)} className="text-slate-300 hover:text-rose-500 p-2 transition-colors"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg></button>
               </div>
             )) : <div className="text-center py-16 opacity-30"><p className="text-xs font-black uppercase tracking-[0.3em]">No trusted contacts yet</p></div>}
           </div>
        </div>
      </div>
      ...
      <div className="p-10 bg-blue-50/50 rounded-[2.5rem] border border-blue-100 flex items-start gap-6">
         ...
         <div>
           <h5 className="text-base font-black text-blue-900 uppercase tracking-tight mb-1.5">{t.securityGuaranteeTitle}</h5>
           <p className="text-blue-800 text-[14px] font-medium leading-relaxed opacity-80">{t.securityGuaranteeDesc}</p>
         </div>
      </div>
    </div>
  );
